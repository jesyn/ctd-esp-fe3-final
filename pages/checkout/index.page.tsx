...
const CheckoutPage: NextPage<PropsCheckout> = () => {

  const [comicData, setComicData] = React.useState<IComic>();
  ...
  const getComicById = async (id: number) => {
    // Este sercicio le pega a mi api
  ...
  useEffect(() => {
    ...
    if (comicId) {
      getComicById(id).then((res) => {
        console.log(res);
        // const comic = toFrontComic(res.data);
